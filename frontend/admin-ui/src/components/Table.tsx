  ...
  return (
    <div className={cn("overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm", className)}>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200" aria-busy={isLoading}>
              ...
              {columns.map((column) => (
                <th
                  ...
                  className={cn(
                    ...
                    column.align ? alignClassMap[column.align] : alignClassMap.left,
                    column.width,
                  )}
                >
                  {column.header}
          ...
          <tbody className={cn("divide-y divide-slate-200", bodyFont)}>
            {showSkeleton
              ? Array.from({ length: skeletonRowCount }).map((_, skeletonIndex) => (
                  <tr key={`loading-${skeletonIndex}`} className="animate-pulse">
                    {columns.map((column) => (
                      <td
                        key={column.id}
                        className={cn(
                ...
                : data.map((row, index) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      onRowClick && "cursor-pointer transition-colors hover:bg-slate-100 focus-visible:outline-none",
                    )}
                  ...
                  >
                    {columns.map((column) => (
                      ...
                      >
                        {column.cell ? column.cell(row) : column.field ? resolveFieldValue(row, column.field) : null}
                      </td>
                    ))}
                    {hasActions ? (
                      <td className={cn(cellPadding, "text-right")}>{renderActions?.(row)}</td>
                    ) : null}
                  </tr>
                ))}
          </tbody>
